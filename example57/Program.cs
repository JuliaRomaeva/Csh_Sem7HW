...
// // int K = array[rows,columns];
// int CountNumbers(int[,] newArray)
// {
//     int count = 0;
...
//         {
//             int K = newArray[i,j];
//             if(newArray[i,j] == newArray[i +1,j + 1]) count++;
//         }
//     }
//     return count;
// }
// // Write($"{K} -> {CountNumbers(array)}");
// Write($"{CountNumbers(array)}");
...
void SortArray(int[] inArray) // пузырьковыя сортировка
{
...
void PrintData(int[] inArray) // подсчет повторяющихся эл-в
{
    int el = inArray[0];
    int count = 1;
